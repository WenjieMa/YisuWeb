...
/* 判断是否隐藏 */
function isHide(obj){
	return obj.style.display==="none"||obj.style.display===""||(obj.currentStyle&&obj.currentStyle==="none")||(window.computedStyle&&window.computedStyle.display==="none");
}
/* end判断是否隐藏 */
...
function hideObj(obj){
    ...
    t=window.setInterval(change,10);
}
/* div渐渐消失 */
...
function showObj(obj){
   ...
   t2=window.setInterval(change2,10);
}
/* div渐渐出现 */
...
/* div获得opacity */
$(document).ready(function(){/* 你好哇 */morning=document.querySelectorAll(".morning");
var myNow = new Date();
var timeH = toTwo(myNow.getHours());
var timeM = toTwo(myNow.getMonth());
var timeD = toTwo(myNow.getDate());
var strM="";
var strD="";
var uShengri=document.getElementById("uShengri").innerHTML;
var i=5;
while(uShengri.charAt(i)!="-"){
	strM=strM+uShengri.charAt(i);
	i++;
}
i++;
while(uShengri.charAt(i)!="-"){
	strD+=uShengri.charAt(i);
	i++
	if(i==uShengri.length){
		break;
	}
}
if (timeH>=6&&timeH<=9){
	morning[0].style.display="block";
	setTimeout("hideObj(morning[0])",2000);
}else if(timeH>=10&&timeH<=11){
	morning[1].style.display="block";
	setTimeout("hideObj(morning[1])",2000);
}
else if(timeH>=12&&timeH<=14){
	morning[2].style.display="block";
	setTimeout("hideObj(morning[2])",2000);
}
else if(timeH>=15&&timeH<=17){
	morning[3].style.display="block";
	setTimeout("hideObj(morning[3])",2000);
}
else if(timeH==18){
	morning[4].style.display="block";
	setTimeout("hideObj(morning[4])",2000);
}
else if(timeH>=19&&timeH<=22){
	morning[5].style.display="block";
	setTimeout("hideObj(morning[5])",2000);
}
else if(timeH>=22&&timeH<=1){
	morning[6].style.display="block";
	setTimeout("hideObj(morning[6])",2000);
}
else if(timeH>=2&&timeH<=5){
	morning[7].style.display="block";
	setTimeout("hideObj(morning[7])",2000);
}else if(strM==timeM&&strD==timeD){
	morning[8].style.display="block";
	setTimeout("hideObj(morning[8])",2000);
}
for (var j = 0; j < 9; j++) {
	setTimeout("deleteDiv(morning["+j+"])",3000);
}
/* end你好哇 */
var search = document.getElementsByName("dong");
for (var i = 0; i < search.length; i++) {
	search[0].onmouseover = function() {
		search[0].style.margin = "-3px 20px";
	}
	search[0].onmouseout = function() {
		search[0].style.margin = "0px 20px";
	}
}
xuan = document.querySelectorAll(".zoukai");
xuanxianglan = document.querySelectorAll(".shouye_biao_xuanxiang");

var tiaoxuan = [ '来场酣畅淋漓的慢跑？---newbanlance', '逼格er than 逼格er ---iphone7',
		'I will be back---终结者4 for XBox360', '静下心来画幅画,试试搜索“梵高”画笔',
		'冰灯玉露,乌木,日月潭---多肉植物' ]
var s_random = Math.random() * 10000 + '';
var num_random = s_random.substring(1, 3) * 1;
document.getElementById("sousuoneirong").placeholder = tiaoxuan[num_random
		% tiaoxuan.length];
setting = {
	width : 1000,
	height : 270,
...
/*
 * textillate.js http://jschr.github.com/textillate MIT licensed
 * 
 ...
 * ! Lettering.JS 0.6.1
 * 
 * Copyright 2010, Dave Rupert http://daverupert.com Released under the WTFPL
...
(function($){
	...
	var methods = {
		...
		lines : function() {
			...
			return this.each(function() {
				...
				// Because it's hard to split a <br/> tag consistently across
				// browsers,
				// (*ahem* IE *ahem*), we replaces all <br/> instances with an
				// md5 hash
				// (of the word "split"). If you're trying to use this plugin on
				// that
				// md5 hash string, it will fail because you're being
				// ridiculous.
	...
	$.fn.lettering = function( method ) {
		...
		if ( method && methods[method] ) {
			...
			return methods.init.apply( this, [].slice.call( arguments, 0 ) ); // always
																				// pass
																				// an
																				// array
		}
...
;(function($){
    ...
    Carousel.prototype = {
        ...
        setDirect:function(){
            ...
            t=window.setInterval('zhuan()',8000);
...
function zhuan() {
    var btn=document.querySelectorAll('.btn-img');
	btn[1].click();
